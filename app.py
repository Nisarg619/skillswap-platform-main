...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS swap_requests (
            ...
            meet_link TEXT, -- Google Meet link for communication
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS notifications (
            ...
            title TEXT NOT NULL,
            message TEXT NOT NULL,
            type TEXT NOT NULL, -- 'swap_request', 'request_accepted', 'request_rejected', 'request_completed'
...
def create_swap_request():
    ...
    try:
        ...
        create_notification(
            receiver_id,
            "New Swap Request",
            f"{sender_name} wants to swap '{skill_offered}' for '{skill_wanted}' with you.",
            "swap_request"
        )
...
def generate_meet_link():
    """Generate a unique Jitsi Meet link that works for immediate use.
    Jitsi requires room names to be alphanumeric (no spaces/special chars)
    and long/unique enough to pass 'invalid video call name' validation."""
    # Use UUID hex: 32 lowercase hex chars, no ambiguous or unsafe words
...
def create_notification(user_id, title, message, notification_type):
    ...
    try:
        ...
        print(f"Database error creating notification: {str(e)}")
        return False
    finally:
        conn.close()
...
def update_swap_request_status(request_id):
    ...
    try:
        ...
        # Generate a video call link if the status is being changed to 'accepted'
        meet_link = req['meet_link']
        if new_status == 'accepted' and not req['meet_link']:
            meet_link = generate_meet_link()

        # Update the status and potentially the meet link
        if meet_link:
            cursor.execute(
                "UPDATE swap_requests SET status = ?, meet_link = ? WHERE id = ?",
                (new_status, meet_link, request_id)
            )
        else:
            cursor.execute(
        ...
        if new_status == 'accepted':
            ...
            create_notification(
                req['sender_id'],
                "Request Accepted!",
                f"{req['receiver_name']} has accepted your request to swap '{req['skill_offered']}' for '{req['skill_wanted']}'.",
                "request_accepted"
            )
            ...
            create_notification(
                req['sender_id'],
                "Request Rejected",
                f"{req['receiver_name']} has rejected your request to swap '{req['skill_offered']}' for '{req['skill_wanted']}'.",
                "request_rejected"
            )
            ...
            create_notification(
                req['sender_id'],
                "Swap Completed",
                f"The swap request for '{req['skill_offered']}' and '{req['skill_wanted']}' has been marked as completed.",
                "request_completed"
            )
            create_notification(
                req['receiver_id'],
        ...
        return jsonify({"message": f"Swap request status updated to {new_status}", "meetLink": meet_link}), 200
    except sqlite3.Error as e:
        conn.rollback()
...
@app.route('/api/completed_swaps', methods=['GET'])
def get_completed_swaps():
    """Get all completed swaps for showcasing successful exchanges."""
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM swap_requests WHERE status = 'completed' ORDER BY created_at DESC LIMIT 20")  # Limit to last 20 completed swaps
        completed_swaps = cursor.fetchall()
        return jsonify([dict(swap) for swap in completed_swaps]), 200
    except sqlite3.Error as e:
        print(f"Database error fetching completed swaps: {str(e)}")
        return jsonify({"error": f"Database error: {str(e)}"}), 500
    finally:
        conn.close()
...
def get_user_notifications(user_id):
    ...
    try:
        ...
        print(f"Database error fetching notifications: {str(e)}")
        return jsonify({"error": f"Database error: {str(e)}"}), 500
    finally:
        conn.close()
...
def get_unread_notifications(user_id):
    ...
    try:
        ...
        print(f"Database error fetching unread notifications: {str(e)}")
        return jsonify({"error": f"Database error: {str(e)}"}), 500
    finally:
        conn.close()

@app.route('/api/notifications/<user_id>/mark_as_read', methods=['PUT'])
def mark_notifications_as_read(user_id):
    ...
    try:
        ...
        conn.rollback()
        print(f"Database error marking notifications as read: {str(e)}")
        return jsonify({"error": f"Database error: {str(e)}"}), 500
    finally:
        conn.close()
